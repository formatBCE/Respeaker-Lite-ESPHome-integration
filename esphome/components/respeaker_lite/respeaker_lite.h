...
enum RespeakerLiteUpdaterStatus : uint8_t {
  UPDATE_OK,
...
class RespeakerLite : public i2c::I2CDevice, public Component {
 public:
  ...
  void add_on_state_callback(std::function<void(DFUAutomationState, float, RespeakerLiteUpdaterStatus)> &&callback) {
 ...
 protected:
 #ifdef USE_RESPEAKER_LITE_STATE_CALLBACK
  CallbackManager<void(DFUAutomationState, float, RespeakerLiteUpdaterStatus)> state_callback_{};
#endif
  RespeakerLiteUpdaterStatus dfu_update_send_block_();
  ...
  RespeakerLiteUpdaterStatus dfu_update_status_{UPDATE_OK};
};
